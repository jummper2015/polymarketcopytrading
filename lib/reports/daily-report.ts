...
export async function generateDailyReport(
  ...
  const todayDecisions = await db
    ...
    .where(
      // Use between for timestamp columns — Drizzle handles Unix integers
      between(decisionJournals.createdAt as any, start as any, end as any)
    );
...
async function getRuleChangeSummaries(
  ...
  const changes = await db
    ...
    .where(
      // Use between for timestamp columns — Drizzle handles Unix integers
      between(ruleChanges.createdAt as any, start as any, end as any)
    )
...
export function formatReportForTelegram(
  ...
  // Header
  lines.push(`📊 *Hermes Daily Report* — ${report.date}`);
  lines.push("");
  ...
  // Footer
  lines.push("_🤖 Generated by Hermes Copy Trading Bot_");
